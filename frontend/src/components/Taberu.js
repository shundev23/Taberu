    ...
    useEffect(() =>{
        const fetchData = async() =>{
            ...
            try{
                ...
                .then(error => console.log('エラー:', error));
                if(!response.ok){
                    throw new Error("サーバーエラー");
                }
                const data = await response.json();

                if(data && data.taberuTop){
                    console.log("pass1");
                    setTaberuTop(data.taberuTop);
                }
                if(data && data.taberuList){
                    console.log("pass2");
                    setTaberuList(data.taberuList);
                }
            }
            catch(error){
                console.log("error");
        ...
        {taberuTop ?(
            ...
            <div className="col-lg-7">
              <img 
                ...
                src={taberuTop.image ? taberuTop.image.url : 'path/to/static/images/main_image.jpg'} 
                alt=""
              />
            </div>
